...
│   └── timeline
├── resources
│   └── get or update <course name/abbreviation/'all'>
├── cron
├── open
...
#### `resources`
Does things with the resources of the course (see the `resources` attribute). Resources are links to (usually PDF) files on the web that are periodically updated. The script offers functionality to do this relatively easily.

##### `get or update <course name/abbreviation>`
Check a given course for resources and update them, if the versions on the URL don't match the ones here. If no course name or abbreviation is specified, all of the courses are checked for resources.
